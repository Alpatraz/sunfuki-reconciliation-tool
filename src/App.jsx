...
function parseShopify(rows){const out=[];for(const row of rows){const order=normalizeOrder(pick(row,["Commande","Order","Name","orderName","Order name","Numéro"]));const statusRaw=pick(row,["Statut commande","Status","Financial status","Fulfillment status","Statut"]);const client=pick(row,["Client","Customer","Nom client"]);const competitor=pick(row,["Compétiteur","Competitor","Athlete","Nom compétiteur","Nom du compétiteur"])||client;const email=pick(row,["Courriel compétiteur","Email","Courriel","Customer email","recipient_email"]);const dojo=pick(row,["Dojo","École","Ecole","Location"]);const team=pick(row,["Équipe","Equipe","Team"]);const rawProduct=pick(row,["Produit","Product","Line item name","Title","Item"]);const qty=Number((pick(row,["Quantité","Quantity","Qty"])||"1").replace(",","."))||1;const refunded=Number((pick(row,["Qté remboursée","Qte remboursee","Refunded quantity","quantityRefunded"])||"0").replace(",","."))||0;const er=pick(row,["Qté effective","Qte effective","Effective quantity"]);let effectiveQty=er===""?Math.max(0,qty-refunded):Number(er.replace(",","."));if(Number.isNaN(effectiveQty))effectiveQty=Math.max(0,qty-refunded);const prod=supplierProductKey(rawProduct);if(!rawProduct)continue;const cancelled=norm(statusRaw).includes("annul")||norm(statusRaw).includes("rembours");const excluded=cancelled||effectiveQty<=0;if(prod==="engagement"){out.push({kind:"engagement",order,competitor,ckey:competitorKey(competitor),client,email,dojo,team,rawProduct,productKey:supplierProductKey(rawProduct,prod),product:supplierProductName(rawProduct,prod),shopifySize:"",quantity:qty,refunded,effectiveQty,excluded:true,statusRaw,status:"ENGAGEMENT",comments:""});continue;}const explicitSize=pick(row,["Taille","Size","Variant","Option","Correction taille"]);const shopifySize=hasSize(prod)?extractSize(explicitSize):"Taille unique";let shopifySizeRaw=findShopifySizeRaw(row,rawProduct,prod);const shopifySizeAnalysis=analyzeSize(shopifySizeRaw||shopifySize,"Shopify");out.push({kind:"product",order,competitor,ckey:competitorKey(competitor),client,email,dojo,team,rawProduct,productKey:supplierProductKey(rawProduct,prod),product:supplierProductName(rawProduct,prod),shopifySizeRaw,shopifySize,shopifySizeStatus:shopifySizeAnalysis.status,quantity:qty,refunded,effectiveQty,excluded,statusRaw,status:excluded?"ANNULÉ / REMBOURSÉ - exclu fournisseur":"",comments:""});}return out;}
function parseSupabase(rows){const out=[];for(const row of rows){const orderRaw=pick(row,["order_number","Commande","Order","orderName","Numéro"]);const orders=splitOrders(orderRaw);const competitor=pick(row,["competitor","Compétiteur","Client","Nom","customerName"]);const email=pick(row,["recipient_email","Email","Courriel","customer_email"]);const rawProduct=pick(row,["product_name","Produit","product","Article","item","Nom produit"]);const sizeRaw=pick(row,["confirmed_size","Taille","size","Nouvelle taille","Réponse"]);const size=normalizeLooseSize(sizeRaw);const sizeAnalysis=analyzeSize(sizeRaw,"Supabase");const comments=pick(row,["comments","Commentaire","comment","Notes","Message","Réponse","response"]);const prod=supplierProductKey(rawProduct);const kind=prod==="engagement"?"engagement":(prod==="unknown"?"comment":"product");const base={kind,orderRaw:normalizeOrder(orderRaw),competitor,ckey:competitorKey(competitor),email,rawProduct,productKey:supplierProductKey(rawProduct,prod),product:supplierProductName(rawProduct,prod),sizeRaw,size,sizeStatus:sizeAnalysis.status,comments};if(orders.length)orders.forEach(order=>out.push({...base,order}));else out.push({...base,order:""});}return out;}
...
function engagementCorrectionKey(order,competitor,team){return `${normalizeOrder(order)}|${competitorKey(competitor)}|${norm(team)}`;}
...
function stripProductNoise(raw=""){
  ...
  s=s.replace(/\s+Équipe\s+Cobra\s*/gi,"");
  s=s.replace(/\s+Equipe\s+Cobra\s*/gi,"");
  s=s.replace(/\s+Cobra\s+International\s*/gi,"");
  s=s.replace(/\s+International\s+Cobra\s*/gi,"");
  s=s.replace(/\s+Coach\s*/gi,"");
  s=s.replace(/\s+Assistant\s+coach\s*/gi,"");
  s=s.replace(/\s+Assistant Coach\s*/gi,"");
  s=s.replace(/\s+Sunfuki\s*/gi,"");
  s=s.replace(/\s+[-–—]\s*$/g,"");
...
function supplierProductName(raw="", fallback=""){
  ...
  // Kimonos : conserver impérativement les familles
  ...
  if(n.includes("noir") && n.includes("croise")) return "Kimono noir croisé";
  if(n.includes("noir") && n.includes("croisé")) return "Kimono noir croisé";
  if(n.includes("blanc") && (n.includes("100") || n.includes("coton"))) return "Kimono blanc 100% coton";
  if(n.includes("blanc")) return "Kimono blanc";
  if(n.includes("kimono")) return stripProductNoise(r).replace(/\s*[-–—]\s*$/,"") || "Kimono";
...
async function saveManualSize(order,productKey,competitor,rawProduct){
  ...
  try{
    ...
    await saveManualSizeToSupabase({order_number:order,product_key:productKey,competitor:competitor||"",raw_product:rawProduct||"",size_raw:value,size_normalized:normalized,source:"manuel"});
    setManualSizeInputs(p=>({...p,[k]:""}));
    await refreshManualSizes();
    setManualParticipantStatus("Taille manuelle sauvegardée.");
  }catch(e){setManualParticipantStatus(e.message||"Erreur sauvegarde taille");}
}
...
const manualReconciliationRows=useMemo(()=>{
  ...
  const engagementOrders=[];

  reconciled.forEach(r=>{
    const manual=manualLinks.find(l=>l.order_number===r.order);
    const linkedName=manual?.fitofan_competitor;
    const effectiveName=linkedName||r.competitor;
    const effectiveKey=competitorKey(effectiveName);

    if(isEngagementOrderRow(r)){
      if(effectiveKey)signedKeys.add(effectiveKey);
      engagementOrders.push({
        Commande:r.order,
        "Nom Shopify":r.manualOriginalCompetitor||r.competitor||"",
        Email:r.email||"",
        Dojo:r.dojo||"",
        Équipe:r.team||"",
        "Produit brut":r.rawProduct||"",
        "Déjà liée":linkedOrdersSet.has(r.order)?"Oui":"Non"
      });
    }
  });

  const missingAthletes=fitofan.competitors.filter(f=>!signedKeys.has(f.key));

  return missingAthletes.map(f=>{
    const suggestions=engagementOrders
      .filter(o=>o["Déjà liée"]!=="Oui")
      .map(o=>({...o,Score:simpleNameScore(f.competitor,o["Nom Shopify"])}))
    ...
    return {
      ...
      "Commandes engagement possibles":suggestions.map(s=>`${s.Commande} - ${s["Nom Shopify"]} (${s.Score}%)`).join(" | ")
    };
  });
},[reconciled,fitofan,manualLinks,linkedOrdersSet]);const manualLinksRows=manualLinks.map(l=>({Commande:l.order_number,"Nom Shopify":l.shopify_competitor,"Compétiteur Fitofan":l.fitofan_competitor,Dojo:l.fitofan_dojo,Équipe:l.fitofan_team,Créé:l.created_at||"",Action:"Supprimer"}));const finalParticipantsRows=useMemo(()=>fitofan.competitors.map(f=>{const rows=reconciled.filter(r=>r.ckey===f.key);const products=rows.filter(r=>r.kind==="product"&&!r.excluded);const engagements=rows.filter(r=>isEngagementOrderRow(r));return{Compétiteur:f.competitor,Email:f.email,Dojo:f.dojo,Équipe:f.team,Source:f.manual?"Ajout manuel":"Fitofan","Engagement signé":engagements.length?"Oui":"Non","Commandes liées":[...new Set(rows.map(r=>r.order).filter(Boolean))].join(" | "),"Produits commandés":products.map(r=>`${r.product} (${r.finalSize||"taille manquante"})`).join(" | "),"Tailles Shopify":products.map(r=>`${r.order} - ${r.product}: ${r.shopifySize||"vide"}`).join(" | "),"Tailles Supabase":products.map(r=>`${r.order} - ${r.product}: ${r.supabaseSize||"aucune"}`).join(" | "),"Tailles manuelles":products.map(r=>`${r.order} - ${r.product}: ${r.manualSizeRaw||"aucune"}`).join(" | "),"Tailles finales":products.map(r=>`${r.order} - ${r.product}: ${r.finalSize||"taille manquante"} (${r.sourceSize||"aucune source"})`).join(" | "),"Tailles manquantes":products.filter(r=>r.missing).length,Commentaires:[...new Set(rows.map(r=>r.comments).filter(Boolean))].join(" | ")};}),[fitofan,reconciled]);const sizeValidationRows=useMemo(()=>reconciled.filter(r=>sizeNeedsValidation(r)).map(r=>({Compétiteur:r.competitor,Dojo:r.dojo,Équipe:r.team,Commande:r.order,Produit:r.product,"Produit normalisé":r.productKey,"Produit Shopify brut":r.rawProduct,"Taille Shopify":r.shopifySize||"","Taille Supabase":r.supabaseSize||"","Taille manuelle":r.manualSizeRaw||"","Taille finale":r.finalSize||"","Source taille":r.sourceSize||"","Statut validation":sizeValidationStatus(r),Commentaires:r.comments||""})),[reconciled]);
const supplierFinalRows=useMemo(()=>{const map=new Map();reconciled.filter(r=>isSupplierReady(r)).forEach(r=>{const key=`${r.product}|${r.team||"Non précisée"}|${r.finalSize}`;const prev=map.get(key)||{Produit:r.product,Équipe:r.team||"Non précisée",Taille:r.finalSize,Quantité:0};prev.Quantité+=Number(r.effectiveQty)||0;map.set(key,prev);});return Array.from(map.values()).sort((a,b)=>`${a.Produit}${a.Équipe}${a.Taille}`.localeCompare(`${b.Produit}${b.Équipe}${b.Taille}`));},[reconciled]);
const supplierInternalRows=useMemo(()=>reconciled.filter(r=>r.kind==="product"&&!r.excluded).map(r=>({Commande:r.order,Compétiteur:r.competitor,Dojo:r.dojo,Équipe:r.team,Produit:r.product,"Produit brut":r.rawProduct,"Taille Shopify":r.shopifySize||"","Taille Supabase":r.supabaseSize||"","Taille manuelle":r.manualSizeRaw||"","Taille finale":r.finalSize||"","Source taille":r.sourceSize||"","Statut validation":sizeValidationStatus(r),Quantité:r.effectiveQty,Commentaires:r.comments||""})),[reconciled]);
...
const engagementOrders=useMemo(()=>reconciled.filter(r=>isEngagementOrderRow(r)).map(r=>({Commande:r.order,"Nom Shopify":r.manualOriginalCompetitor||r.competitor||"",Compétiteur:r.competitor,Dojo:r.dojo,Équipe:r.team,"Produit brut":r.rawProduct||""})),[reconciled]);
const engagementManagementRows=useMemo(()=>{const rows=[];fitofan.competitors.forEach(f=>{const related=engagementOrders.filter(e=>((competitorKey(e.Compétiteur)===f.key)&&norm(e.Équipe)===norm(f.team))||engagementCorrectionsMap.link.has(engagementCorrectionKey(e.Commande,f.competitor,f.team))).filter(e=>!engagementCorrectionsMap.unlink.has(engagementCorrectionKey(e.Commande,f.competitor,f.team)));if(related.length){related.forEach(e=>rows.push({Compétiteur:f.competitor,Dojo:f.dojo,Équipe:f.team,Commande:e.Commande,"Nom Shopify":e["Nom Shopify"],Statut:"LIÉ",Action:"Délier"}));}else{rows.push({Compétiteur:f.competitor,Dojo:f.dojo,Équipe:f.team,Commande:"","Nom Shopify":"",Statut:"AUCUN ENGAGEMENT LIÉ",Action:"Lier"});}});return rows;},[fitofan,engagementOrders,engagementCorrectionsMap]);
const engagementManagementFilteredRows=useMemo(()=>{const q=norm(engagementSearch);return engagementManagementRows.filter(r=>{const txt=norm(`${r.Compétiteur} ${r.Dojo} ${r.Équipe} ${r.Commande} ${r["Nom Shopify"]} ${r.Statut}`);const okSearch=!q||txt.includes(q);const okStatus=engagementStatusFilter==="all"||(engagementStatusFilter==="linked"&&r.Statut==="LIÉ")||(engagementStatusFilter==="missing"&&r.Statut!=="LIÉ");return okSearch&&okStatus;});},[engagementManagementRows,engagementSearch,engagementStatusFilter]);const engagementCorrectionsRows=engagementCorrections.map(c=>({ID:c.id,Commande:c.order_number,Compétiteur:c.competitor,Équipe:c.team,Action:c.action,Créé:c.created_at||""}));
const correctedMissingEngagementRows=useMemo(()=>engagementManagementRows.filter(r=>r.Statut!=="LIÉ").map(r=>({Compétiteur:r.Compétiteur,Dojo:r.Dojo,Équipe:r.Équipe,Engagement:"MANQUANT","Action suggérée":"Gestion liens engagements"})),[engagementManagementRows]);const views={dashboard:{title:"Tableau de bord visuel",rows:dashboardFiltered},engagementManagement:{title:"Gestion liens engagements",rows:engagementManagementFilteredRows},engagementCorrections:{title:"Corrections engagements sauvegardées",rows:engagementCorrectionsRows},sizeValidation:{title:"Validation tailles",rows:sizeValidationRows},supplierFinal:{title:"Export fournisseur final",rows:supplierFinalRows},supplierInternal:{title:"Export interne détaillé",rows:supplierInternalRows},participantsFinal:{title:"Export final participants",rows:finalParticipantsRows},manualParticipants:{title:"Ajouter / gérer participants",rows:manualCompetitors.map(r=>({Compétiteur:r.competitor,Email:r.email,Dojo:r.dojo,Équipe:r.team,Source:r.source||"manuel"}))},manualReconciliation:{title:"Engagements manquants à réconcilier",rows:manualReconciliationRows},manualLinks:{title:"Liens manuels sauvegardés",rows:manualLinksRows},unusualSizes:{title:"Tailles inhabituelles à vérifier",rows:unusualSizeRows},shopifyInitialMissing:{title:"Tailles Shopify manquantes initialement",rows:shopifyInitialMissingRows},missingSizes:{title:"Compétiteurs avec tailles manquantes",rows:competitorsMissingSizes},missingEngagements:{title:"Engagements manquants",rows:correctedMissingEngagementRows},audit:{title:"Contrôle qualité",rows:auditRows},detail:{title:"Détail global Shopify + Supabase",rows:detailRows},fitofan:{title:"Suivi par compétiteur Fitofan",rows:fitofanReport},supplier:{title:"Commande fournisseur",rows:supplierRows},supabase:{title:"Audit réponses Supabase",rows:supabaseAudit},fitofanComments:{title:"Commentaires Fitofan",rows:fitofan.comments}};const tabGroups=[
...
{id:"sizes",label:"Tailles",tabs:["sizeValidation","missingSizes","unusualSizes","shopifyInitialMissing"]},
{id:"supplier",label:"Fournisseur",tabs:["supplierFinal","supplier","supplierInternal"]},
{id:"participants",label:"Participants",tabs:["fitofan","participantsFinal","manualParticipants"]},
{id:"audit",label:"Audit / contrôle",tabs:["audit","detail","supabase","fitofanComments"]}
];
...
return <div className="app"><header><h1>Réconciliation Sunfuki V30.4</h1><p>Sources colorées · alertes compétiteurs · engagements manquants · exports filtrés par club.</p></header><section className="sourceStatusGrid">
