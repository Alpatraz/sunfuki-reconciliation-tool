...
function parseShopify(rows){const out=[];for(const row of rows){const order=normalizeOrder(pick(row,["Commande","Order","Name","orderName","Order name","Numéro"]));const statusRaw=pick(row,["Statut commande","Status","Financial status","Fulfillment status","Statut"]);const client=pick(row,["Client","Customer","Nom client"]);const competitor=pick(row,["Compétiteur","Competitor","Athlete","Nom compétiteur","Nom du compétiteur"])||client;const email=pick(row,["Courriel compétiteur","Email","Courriel","Customer email","recipient_email"]);const dojo=pick(row,["Dojo","École","Ecole","Location"]);const team=pick(row,["Équipe","Equipe","Team"]);const rawProduct=pick(row,["Produit","Product","Line item name","Title","Item"]);const qty=Number((pick(row,["Quantité","Quantity","Qty"])||"1").replace(",","."))||1;const refunded=Number((pick(row,["Qté remboursée","Qte remboursee","Refunded quantity","quantityRefunded"])||"0").replace(",","."))||0;const er=pick(row,["Qté effective","Qte effective","Effective quantity"]);let effectiveQty=er===""?Math.max(0,qty-refunded):Number(er.replace(",","."));if(Number.isNaN(effectiveQty))effectiveQty=Math.max(0,qty-refunded);const itemTeam=productTeamFromTitle(rawProduct);const saikoVariant=saikoVariantInfo(rawProduct,itemTeam);const prod=saikoVariant?.key||supplierProductKey(rawProduct);if(!rawProduct)continue;const cancelled=norm(statusRaw).includes("annul")||norm(statusRaw).includes("rembours");const excluded=cancelled||effectiveQty<=0;if(prod==="engagement"){out.push({kind:"engagement",order,competitor,ckey:competitorKey(competitor),client,email,dojo,team,rawProduct,productKey:prod,product:saikoVariant?.name||supplierProductName(rawProduct,prod),productTeam:saikoVariant?.productTeam||itemTeam,shopifySize:"",quantity:qty,refunded,effectiveQty,excluded:true,statusRaw,status:"ENGAGEMENT",comments:""});continue;}const explicitSize=pick(row,["Taille","Size","Variant","Option","Correction taille"]);const shopifySize=hasSize(prod)?extractSize(explicitSize):"Taille unique";let shopifySizeRaw=findShopifySizeRaw(row,rawProduct,prod);const shopifySizeAnalysis=analyzeSize(shopifySizeRaw||shopifySize,"Shopify");out.push({kind:"product",order,competitor,ckey:competitorKey(competitor),client,email,dojo,team,rawProduct,productKey:prod,product:saikoVariant?.name||supplierProductName(rawProduct,prod),productTeam:saikoVariant?.productTeam||itemTeam,shopifySizeRaw,shopifySize,shopifySizeStatus:shopifySizeAnalysis.status,quantity:qty,refunded,effectiveQty,excluded,statusRaw,status:excluded?"ANNULÉ / REMBOURSÉ - exclu fournisseur":"",comments:""});}return out;}
...
async function loadManualEquipmentOrdersFromSupabase(){
  const url=import.meta.env.VITE_SUPABASE_URL;
  const key=import.meta.env.VITE_SUPABASE_ANON_KEY;
  if(!url||!key)throw new Error("Variables Supabase manquantes.");
  const endpoint=`${url.replace(/\/$/,"")}/rest/v1/manual_equipment_orders?select=*&order=created_at.desc`;
  const res=await fetch(endpoint,{headers:{apikey:key,Authorization:`Bearer ${key}`,"Content-Type":"application/json"}});
  if(!res.ok){const text=await res.text();throw new Error(`Erreur chargement commandes internes ${res.status}: ${text}`);}
  return await res.json();
}
async function saveManualEquipmentOrderToSupabase(row){
  const url=import.meta.env.VITE_SUPABASE_URL;
  const key=import.meta.env.VITE_SUPABASE_ANON_KEY;
  if(!url||!key)throw new Error("Variables Supabase manquantes.");
  const endpoint=`${url.replace(/\/$/,"")}/rest/v1/manual_equipment_orders`;
  const res=await fetch(endpoint,{method:"POST",headers:{apikey:key,Authorization:`Bearer ${key}`,"Content-Type":"application/json",Prefer:"return=representation"},body:JSON.stringify(row)});
  if(!res.ok){const text=await res.text();throw new Error(`Erreur sauvegarde commande interne ${res.status}: ${text}`);}
  return await res.json();
}
async function deleteManualEquipmentOrderFromSupabase(id){
  const url=import.meta.env.VITE_SUPABASE_URL;
  const key=import.meta.env.VITE_SUPABASE_ANON_KEY;
  if(!url||!key)throw new Error("Variables Supabase manquantes.");
  const endpoint=`${url.replace(/\/$/,"")}/rest/v1/manual_equipment_orders?id=eq.${id}`;
  ...
  if(!res.ok){const text=await res.text();throw new Error(`Erreur suppression commande interne ${res.status}: ${text}`);}
  return true;
}
...
function saikoVariantInfo(raw="", team=""){
  const r=clean(raw);
  const n=norm(r);
  const t=norm(team);
  if(!(n.includes("saiko")||n.includes("saïko")||n.includes("arawaza")))return null;

  const isCobraInternational=n.includes("international cobra")||n.includes("cobra international")||n.includes("equipe international")||n.includes("équipe international")||t.includes("international");
  const isCobra=(n.includes("equipe cobra")||n.includes("équipe cobra")||t==="cobra"||t.includes(" cobra")) && !isCobraInternational;

  if(isCobraInternational){
    return {name:"Kimono Saïko (Arawaza) - Équipe Cobra International", key:"kimono saiko arawaza equipe cobra international", productTeam:"Cobra International"};
  }
  if(isCobra){
    return {name:"Kimono Saïko (Arawaza) - Équipe Cobra", key:"kimono saiko arawaza equipe cobra", productTeam:"Cobra"};
  }

  return {name:"Kimono Saïko (Arawaza)", key:"kimono saiko arawaza sans broderie", productTeam:""};
}
...
function supplierProductName(raw="", fallback=""){
  ...
  if(n.includes("saiko")||n.includes("saïko")||n.includes("arawaza")) return saikoVariantInfo(r)?.name || "Kimono Saïko (Arawaza)";
...
function supplierProductKey(raw="", fallback=""){
  const variant=saikoVariantInfo(raw);
...
function productTeamFromTitle(raw=""){
  ...
  if(n.includes("coach"))return "Coach";
  if(n.includes("international cobra") || n.includes("cobra international"))return "Cobra International";
  if(n.includes("equipe international") || n.includes("équipe international"))return "Cobra International";
...
function displayProductTeam(itemTeam="", competitorTeam="", kind="product"){
  ...
  return itemTeam || "ÉQUIPE MANQUANTE";
}


function resolveExportTeam(row={}, fitofanCompetitors=[]){
  const v=saikoVariantInfo(row.rawProduct||row.product||"", row.productTeam||"");
  if(v && !v.productTeam)return "Sans équipe / sans broderie";
...
function displayTeamCategory(value=""){
  ...
  // Exception demandée : coach-propriétaire est regroupé sous Coach.
  if(n.includes("coach")&&(n.includes("proprietaire")||n.includes("propriétaire")||n.includes("owner")))return "Coach";

  if(n.includes("assistant"))return "Assistant-Coach";
  if(n.includes("coach"))return "Coach";

  // Unifier toutes les variantes International / International Cobra / Cobra International.
  if(n.includes("international")||n.includes("inter"))return "Cobra International";

  if(n.includes("cobra"))return "Cobra";

  // Propriétaires : garder les catégories spécifiques pour les produits distincts.
  if(n.includes("proprietaire")||n.includes("propriétaire")||n.includes("owner")){
    if(n.includes("sunfuki"))return "Propriétaire Sunfuki";
    if(n.includes("vendeur"))return "Propriétaire vendeur";
    if(n.includes("famille"))return "Propriétaire famille";
    return raw||"Propriétaire";
  }
...
function manualSizeLineKey(row={}){
  const order=normalizeOrder(row.order||row.Commande||row.order_number||"");
  const productKey=clean(row.productKey||row.product_key||row.Produit||row.product||"");
  const raw=clean(row.rawProduct||row.raw_product||row["Produit Shopify brut"]||"");
  const shopifySize=clean(row.shopifySize||row.shopify_size||row["Taille Shopify"]||"");
  const unique=clean(row.uniqueKey||row.unique_key||"");
  const rawPart=raw||unique||productKey;
  return `${order}|line:${productKey}|${rawPart}|${shopifySize}`;
}
function manualSizeLegacyKey(row={}){
  const order=normalizeOrder(row.order||row.Commande||row.order_number||"");
  const productKey=clean(row.productKey||row.product_key||row.Produit||row.product||"");
  return `${order}|${productKey}`;
}
function getManualSizeOverride(row={},manualSizeMap){
  return manualSizeMap.get(manualSizeLineKey(row)) || manualSizeMap.get(manualSizeLegacyKey(row));
}


function internalOrderNumber(row={},idx=0){
  return row.order_number||row.orderNumber||`INT-${row.id||idx+1}`;
}
function boolValue(v,defaultValue=true){
  if(v===undefined||v===null||v==="")return defaultValue;
  if(typeof v==="boolean")return v;
  const n=norm(String(v));
  if(["false","non","no","0"].includes(n))return false;
  if(["true","oui","yes","1"].includes(n))return true;
  return defaultValue;
}
function manualEquipmentToShopifyRow(row={},idx=0){
  const raw=row.raw_product||row.rawProduct||row.product||row.product_name||"";
  const variant=saikoVariantInfo(raw,row.team||"");
  const productKey=variant?.key||row.product_key||supplierProductKey(raw);
  const productName=variant?.name||row.product||supplierProductName(raw,productKey);
  const size=normalizeManualSizeForSave(row.size_normalized||row.size_raw||row.size||"");
  const qty=Number(row.quantity)||1;
  const includeSupplier=boolValue(row.include_supplier,true);
  const includeClient=boolValue(row.include_client,false);
  return {
    kind:"product",
    order:internalOrderNumber(row,idx),
    competitor:row.competitor||"",
    ckey:competitorKey(row.competitor||""),
    client:row.client||row.competitor||"",
    email:row.email||"",
    dojo:row.dojo||"",
    team:row.team||"",
    rawProduct:raw,
    productKey,
    product:productName,
    productTeam:variant?.productTeam||row.product_team||productTeamFromTitle(raw),
    shopifySizeRaw:"",
    shopifySize:size,
    shopifySizeStatus:size?"STANDARD":"MANQUANTE",
    quantity:qty,
    refunded:0,
    effectiveQty:qty,
    excluded:!includeSupplier,
    includeSupplier,
    includeClient,
    internalOrder:true,
    statusRaw:"Commande interne hors Shopify",
    status:includeSupplier?"COMMANDE INTERNE - FOURNISSEUR":"COMMANDE INTERNE - HORS FOURNISSEUR",
    comments:row.notes||"",
    uniqueKey:`internal|${row.id||idx}|${row.competitor||""}|${raw}|${size}`
  };
}


function internalProductCatalog(){
  return [
    "T-shirt",
    "Kangourou / Hoodie",
    "Veste",
    ...
    "Kimono noir croisé",
    "Kimono blanc",
    "Kimono Noir 100% coton",
    "Kimono blanc 100% coton",
    "Kimono Saïko (Arawaza)",
    "Kimono Saïko (Arawaza) - Équipe Cobra",
    "Kimono Saïko (Arawaza) - Équipe Cobra International",
    "Gants",
    "Protège-pieds",
    "Protège-tibias",
    "Grand sac à gros zipper",
    "Petit sac à gros zipper",
    "Très grand sac à roulette"
  ];
}
function makeInternalProductLine(product){
  return {product,entries:[{size:"",quantity:1}]};
}
function expandInternalEquipmentDraft(form={},draft=[]){
  const rows=[];
  const orderNumber=`INT-${Date.now()}`;
  draft.forEach(line=>{
    (line.entries||[]).forEach(entry=>{
      const qty=Number(entry.quantity)||1;
      const size=clean(entry.size||"");
      if(!clean(line.product)||!size||qty<1)return;
      rows.push({
        order_number:orderNumber,
        competitor:form.competitor||"",
        email:form.email||"",
        dojo:form.dojo||"",
        team:displayTeamCategory(form.team)||form.team||"",
        raw_product:line.product,
        product_key:saikoVariantInfo(line.product,form.team)?.key||supplierProductKey(line.product),
        product:saikoVariantInfo(line.product,form.team)?.name||supplierProductName(line.product,supplierProductKey(line.product)),
        product_team:saikoVariantInfo(line.product,form.team)?.productTeam||productTeamFromTitle(line.product),
        size_raw:size,
        size_normalized:normalizeManualSizeForSave(size),
        quantity:qty,
        include_supplier:!!form.includeSupplier,
        include_client:!!form.includeClient,
        notes:form.notes||""
      });
    });
  });
...
function includeInClientReport(row={}){
  if(row.internalOrder)return !!row.includeClient;
  return true;
}
function includeInSupplierReport(row={}){
  if(row.internalOrder)return !!row.includeSupplier;
  return !row.excluded;
}
...
async function saveManualSize(order,productKey,competitor,rawProduct,itemOverride=null){
  ...
  const lineKey=manualSizeLineKey({...item,order:normalizedOrder,productKey,competitor,rawProduct});
  const productKeyToSave=lineKey.replace(`${normalizedOrder}|`,"");
  ...
  try{
    ...
    const payload={order_number:normalizedOrder,product_key:productKeyToSave,competitor:competitor||"",raw_product:rawProduct||"",size_raw:value,size_normalized:normalized,source:"manuel"};
...
async function refreshManualEquipmentOrders(){
  try{
    const rows=await loadManualEquipmentOrdersFromSupabase();
    setManualEquipmentOrders(rows||[]);
    setManualEquipmentStatus(`Commandes internes chargées : ${(rows||[]).length}`);
  }catch(e){
    setManualEquipmentStatus(e.message||"Table manual_equipment_orders absente");
    console.warn("Chargement commandes internes",e);
  }
}
function selectManualEquipmentCompetitor(name){
  const f=(fitofan?.competitors||[]).find(x=>x.competitor===name)||{};
  setManualEquipmentForm(p=>({...p,competitor:name,email:f.email||p.email||"",dojo:f.dojo||p.dojo||"",team:displayTeamCategory(f.team)||f.team||p.team||""}));
}
async function addManualEquipmentOrder(){
  setManualEquipmentStatus("Sauvegarde commande interne...");
  try{
    if(!clean(manualEquipmentForm.competitor))throw new Error("Compétiteur requis.");
    if(!clean(manualEquipmentForm.product))throw new Error("Produit requis.");
    const qty=Number(manualEquipmentForm.quantity)||1;
    if(qty<1)throw new Error("Quantité invalide.");
    const payload={
      order_number:`INT-${Date.now()}`,
      competitor:manualEquipmentForm.competitor,
      email:manualEquipmentForm.email||"",
      dojo:manualEquipmentForm.dojo||"",
      team:manualEquipmentForm.team||"",
      raw_product:manualEquipmentForm.product,
      product_key:supplierProductKey(manualEquipmentForm.product),
      product:supplierProductName(manualEquipmentForm.product,supplierProductKey(manualEquipmentForm.product)),
      size_raw:manualEquipmentForm.size||"",
      size_normalized:normalizeManualSizeForSave(manualEquipmentForm.size||""),
      quantity:qty,
      include_supplier:!!manualEquipmentForm.includeSupplier,
      include_client:!!manualEquipmentForm.includeClient,
      notes:manualEquipmentForm.notes||""
    };
    const saved=await saveManualEquipmentOrderToSupabase(payload);
    const row=Array.isArray(saved)&&saved[0]?saved[0]:payload;
    setManualEquipmentOrders(prev=>[row,...(prev||[])]);
    setManualEquipmentForm({competitor:"",email:"",dojo:"",team:"",product:"",size:"",quantity:1,includeSupplier:true,includeClient:false,notes:""});
    setManualEquipmentStatus("Commande interne ajoutée et liée au profil.");
  }catch(e){
    setManualEquipmentStatus(e.message||"Erreur commande interne");
    console.error("Erreur commande interne",e);
  }
}

function toggleInternalDraftProduct(product){
  setManualEquipmentDraft(prev=>{
    const exists=(prev||[]).some(l=>l.product===product);
    if(exists)return prev.filter(l=>l.product!==product);
    return [...(prev||[]),makeInternalProductLine(product)];
  });
}
function updateInternalDraftEntry(product,index,field,value){
  setManualEquipmentDraft(prev=>(prev||[]).map(line=>{
    if(line.product!==product)return line;
    const entries=[...(line.entries||[])];
    entries[index]={...(entries[index]||{size:"",quantity:1}),[field]:value};
    return {...line,entries};
  }));
}
function addInternalDraftSizeLine(product){
  setManualEquipmentDraft(prev=>(prev||[]).map(line=>line.product===product?{...line,entries:[...(line.entries||[]),{size:"",quantity:1}]}:line));
}
function removeInternalDraftSizeLine(product,index){
  setManualEquipmentDraft(prev=>(prev||[]).map(line=>{
    if(line.product!==product)return line;
    const entries=(line.entries||[]).filter((_,i)=>i!==index);
    return {...line,entries:entries.length?entries:[{size:"",quantity:1}]};
  }));
}
async function createInternalEquipmentBatch(){
  setManualEquipmentStatus("Création de la commande interne...");
  try{
    if(!clean(manualEquipmentForm.competitor))throw new Error("Compétiteur requis.");
    const rows=expandInternalEquipmentDraft(manualEquipmentForm,manualEquipmentDraft);
    if(!rows.length)throw new Error("Aucun produit avec taille / quantité valide.");
    const savedRows=[];
    ...
    setManualEquipmentOrders(prev=>[...savedRows,...(prev||[])]);
    setManualEquipmentDraft([]);
    setManualEquipmentForm(p=>({...p,product:"",size:"",quantity:1,notes:""}));
    setManualEquipmentStatus(`${savedRows.length} ligne(s) de commande interne ajoutée(s).`);
  }catch(e){
    setManualEquipmentStatus(e.message||"Erreur commande interne");
    console.error("Erreur commande interne rapide",e);
  }
}
async function removeManualEquipmentOrder(id){
  if(!id)return;
  setManualEquipmentStatus("Suppression commande interne...");
  try{
    await deleteManualEquipmentOrderFromSupabase(id);
    setManualEquipmentOrders(prev=>(prev||[]).filter(r=>r.id!==id));
    setManualEquipmentStatus("Commande interne supprimée.");
  }catch(e){
    setManualEquipmentStatus(e.message||"Erreur suppression commande interne");
  }
}
const fitofanBase=useMemo(()=>parseFitofan(fitofanRaw),[fitofanRaw]);const fitofan=useMemo(()=>{const base=fitofanBase.competitors||[];const keys=new Set(base.map(f=>f.key));const manual=(manualCompetitors||[]).map(r=>({competitor:r.competitor,key:competitorKey(r.competitor),email:r.email||"",dojo:r.dojo||"",team:r.team||"",manual:true,fitofanStatus:r.fitofan_status||r.fitofanStatus||"manuel"})).filter(f=>!keys.has(f.key));return{competitors:[...base,...manual],comments:fitofanBase.comments||[]};},[fitofanBase,manualCompetitors]);const shopifyParsed=useMemo(()=>parseShopify(shopifyRaw),[shopifyRaw]);const shopify=shopifyCloudRows.length?shopifyCloudRows:shopifyParsed;const internalEquipmentRows=useMemo(()=>manualEquipmentOrders.map((r,idx)=>manualEquipmentToShopifyRow(r,idx)),[manualEquipmentOrders]);const shopifyWithInternal=useMemo(()=>[...shopify,...internalEquipmentRows],[shopify,internalEquipmentRows]);const supabase=useMemo(()=>parseSupabase(supabaseRaw),[supabaseRaw]);
...
const manualLinkByOrder=useMemo(()=>{const m=new Map();manualLinks.forEach(l=>{if(l.order_number)m.set(l.order_number,l);});return m;},[manualLinks]);const manualSizeMap=useMemo(()=>{
  const m=new Map();
  manualSizes.forEach(s=>{
    if(s.order_number&&s.product_key){
      const k=`${normalizeOrder(s.order_number)}|${s.product_key}`;
...
const fitofanReport=useMemo(()=>fitofan.competitors.map(f=>{const rows=reconciled.filter(r=>r.ckey===f.key);const products=rows.filter(r=>r.kind==="product"&&!r.excluded);const engagementKey=`${f.key}|${canonicalTeamKey(f.team)}`;const engagements=resolvedEngagementState.engagementByCompetitorTeam.get(engagementKey)||[];const orders=[...new Set([...rows.map(r=>r.order).filter(Boolean),...engagements.map(e=>e.order).filter(Boolean)])];return{Compétiteur:f.competitor,Email:f.email,Dojo:f.dojo,Équipe:displayTeamCategory(f.team),FITOFAN:f.fitofanStatus||"","Engagement signé":engagements.length?"Oui":"Non","Commandes liées":orders.join(" | "),"A acheté équipement":products.length?"Oui":"Non","Produits commandés":products.map(r=>`${r.product} (${r.finalSize||"taille manquante"}) x${r.effectiveQty||r.quantity||1}`).join(" | "),"Tailles Shopify":products.map(r=>`${r.order} - ${r.product}: ${r.shopifySize||"vide"} x${r.effectiveQty||r.quantity||1}`).join(" | "),"Tailles Supabase":products.map(r=>`${r.order} - ${r.product}: ${r.supabaseSize||"aucune"} x${r.effectiveQty||r.quantity||1}`).join(" | "),"Tailles finales":products.map(r=>`${r.order} - ${r.product}: ${r.finalSize||"taille manquante"} x${r.effectiveQty||r.quantity||1} (${r.sourceSize||"aucune source"})`).join(" | "),"Tailles manquantes":products.filter(r=>r.missing).length,Commentaires:[...new Set(rows.map(r=>r.comments).filter(Boolean))].join(" | ")};}),[fitofan,reconciled,resolvedEngagementState]);
...
const shopifyUnknownCompetitorRows=useMemo(()=>{
  ...
  (reconciled||[]).forEach(r=>{
    ...
    if(r.kind==="product"&&!r.excluded){
      cur.Équipement=true;
      cur.Produits.push(`${r.product} (${r.finalSize||"taille manquante"}) x${r.effectiveQty||r.quantity||1}`);
    }
    map.set(key,cur);
  });
  ...
  return (fitofan?.competitors||[])
    ...
    .map(f=>{
      ...
      return {
        ...
        Produits:products.map(r=>`${r.product} (${r.finalSize||"taille manquante"}) x${r.effectiveQty||r.quantity||1}`).join(" | ")
      };
    })
...
const visibleSizeValidationRows=useMemo(()=>sizeValidationRows.filter(r=>!savedSizeKeys.has(manualSizeLineKey(r))),[sizeValidationRows,savedSizeKeys]);
...
const clientTotalItemsRows=useMemo(()=>{
  ...
  expandRowsByFinalSizes(reconciled||[])
    .filter(r=>r.kind==="product")
    .filter(r=>includeInClientReport(r))
    ...
    .forEach(r=>{
      ...
      const status=supplierLineStatus(r);
      const key=`${product}|${team}|${size}|${status}`;
      const cur=map.get(key)||{Produit:product,Équipe:team,Taille:size,Statut:status,Quantité:0};
      cur.Quantité += Number(r.effectiveQty||r.quantity||0)||0;
      map.set(key,cur);
    });
  return Array.from(map.values()).sort((a,b)=>String(a.Produit).localeCompare(String(b.Produit))||String(a.Équipe).localeCompare(String(b.Équipe))||String(a.Taille).localeCompare(String(b.Taille)));
...
const quantityByOrderAuditRows=useMemo(()=>expandRowsByFinalSizes(reconciled||[])
  ...
  .map(r=>({
    ...
    Quantité:Number(r.effectiveQty||r.quantity||0)||0,
    Interne:r.internalOrder?"Oui":"Non",
    "Inclus client":includeInClientReport(r)?"Oui":"Non",
    "Inclus fournisseur":includeInSupplierReport(r)?"Oui":"Non"
  })),[reconciled]);
...
const missingTeamRows=useMemo(()=>expandRowsByFinalSizes(reconciled||[])
  ...
  .map(r=>({
    ...
    Taille:r.finalSize||"TAILLE MANQUANTE",
    Quantité:Number(r.effectiveQty||r.quantity||0)||0,
    "Équipe détectée":r._exportTeam,
    Commentaires:r.comments||""
  })),[reconciled,fitofan]);


const saikoAuditRows=useMemo(()=>expandRowsByFinalSizes(reconciled||[])
  .filter(r=>r.kind==="product")
  .filter(r=>norm(r.product).includes("saiko")||norm(r.rawProduct).includes("saiko")||norm(r.rawProduct).includes("saïko")||norm(r.rawProduct).includes("arawaza"))
  .map(r=>({
    Commande:r.order,
    Compétiteur:r.competitor,
    Dojo:r.dojo,
...
{id:"supplier",label:"Fournisseur",tabs:["supplierProduction","clientTotalItems","supplierFinalWithExceptions","supplierAuditSummary","quantityByOrderAudit","saikoAudit","missingTeam","supplierAudit","supplier","supplierInternal"]},
{id:"participants",label:"Participants",tabs:["fitofan","participantsFinal","manualParticipants","internalEquipment"]},
{id:"audit",label:"Audit / contrôle",tabs:["audit","missingTeam","supplierMultiSizeWarning","teamCategoryAudit","productTeamAudit","productsNoProductTeam","shopifyUnknownCompetitors","notFitofanWithOrders","fitofanStatusAudit","detail","supabase","fitofanComments"]}
];
