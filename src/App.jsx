...
function manualEquipmentToShopifyRow(row={},idx=0){
  ...
  const productKey=supplierProductKey(raw);
  ...
  return {
    ...
    rawProduct:raw,
    productKey,
    product:supplierProductName(raw,productKey),
    shopifySizeRaw:"",
    shopifySize:size,
  ...
  draft.forEach(line=>{
    (line.entries||[]).forEach(entry=>{
      ...
      rows.push({
        ...
        product_key:supplierProductKey(line.product),
        product:supplierProductName(line.product,supplierProductKey(line.product)),
        size_raw:size,
...
const teamCategoryAuditRows=useMemo(()=>{
  const all=[
    ...(fitofan?.competitors||[]).map(r=>({Source:"Fitofan",Nom:r.competitor,Équipe:supplierTeamForRow(r),Catégorie:displayTeamCategory(r.team)})),
    ...(reconciled||[]).map(r=>({Source:"Shopify",Nom:r.competitor,Équipe:supplierTeamForRow(r),Catégorie:displayTeamCategory(r.team)}))
  ];
  const map=new Map();
  all.forEach(r=>{
...
{id:"supplier",label:"Fournisseur",tabs:["supplierProduction","clientTotalItems","supplierFinalWithExceptions","supplierAuditSummary","quantityByOrderAudit","supplierAudit","supplier","supplierInternal"]},
...
return <div className="app"><header><h1>Réconciliation Sunfuki V35.1</h1><p>Exports séparés client / fournisseur · équipes normalisées · commandes internes rapides.</p></header><section className="sourceStatusGrid">
